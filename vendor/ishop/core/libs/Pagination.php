...
class Pagination{
    ...
    public function getHtml(){//формирует пагинацию
        ...
        if( $this->currentPage > 1 ){//емсли не находимся на 1 странице
            $back = "<li><a class='nav-link' href='{$this->uri}page=" .($this->currentPage - 1). "'>&lt;</a></li>";
        }
        if( $this->currentPage < $this->countPages ){//ссылка вперед, проверяет то, что она не последняя
    ...
    public function __toString(){//объект конвертируем в строку
        return $this->getHtml();
    }

    public function getCountPages(){//считаем кол-во страниц, на которые делятся продукты 
        return ceil($this->total / $this->perpage) ?: 1;
    }

    public function getCurrentPage($page){
        if(!$page || $page < 1) $page = 1;//0 страницы не может быть
        if($page > $this->countPages) $page = $this->countPages;//если пользователь захочет посотреть страницу, которой нет
        return $page;
    }
    ...
    public function getParams(){
        ...
        $url = explode('?', $url);// разбиваем url строку по знаку вопроса 
        $uri = $url[0] . '?';
